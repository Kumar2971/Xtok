...
import StripeIntegration, { ViewProps } from "../src/StripeIntegration";
...
jest.mock("../src/StripeIntegrationView.tsx", () => (props: ViewProps) => (
  <View {...props} />
));
...
describe("StripeIntegration", () => {
  ...
  it("should pass through the correct intial state values", async () => {
    const { queryByTestId } = render(<StripeIntegration {...screenProps} />);
    const view = queryByTestId("StripeIntegrationView");
  ...
  it("should send the correct network request when btnCreatePaymentMethodProps.onPress is called if card details are set", async () => {
    ...
    await waitFor(() => {
      ...
      expect(sendNetworkRequestMock).toBeCalledWith(
        expect.any(Object),
        "POST",
        "stripe_integration/payment_methods",
  ...
  it("should handle the SessionResponseMessage response", async () => {
    render(<StripeIntegration {...screenProps} />);
  ...
  it("should handle the RestAPIResponseMessage response for the create payment POST", async () => {
    ...
    await waitFor(() => {
      ...
      expect(sendNetworkRequestMock).toBeCalledWith(
        expect.any(Object),
        "POST",
        "stripe_integration/payments/confirm",
        { "Content-Type": "application/json", token: "" },
        {
