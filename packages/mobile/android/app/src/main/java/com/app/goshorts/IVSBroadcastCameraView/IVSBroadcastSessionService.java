...
public class IVSBroadcastSessionService {
    ...
    private Device.Descriptor[] getInitialDeviceDescriptorList() {
        return initialCameraPosition == Device.Descriptor.Position.BACK ? Presets.Devices.BACK_CAMERA(mReactContext) : Presets.Devices.FRONT_CAMERA(mReactContext);
    }
    ...
    private void setCustomVideoConfig() {
        if (customVideoConfig != null) {
            config = config.changing($ -> {
                ...
                if (isWidth || isHeight) {
                    if (isWidth && isHeight) {
                        $.video.setSize(customVideoConfig.getInt("width"), customVideoConfig.getInt("height"));
                    } else {
    ...
    public enum Events {
        ON_ERROR("onError"), ON_STATE_CHANGED("onStateChanged"), ON_AUDIO_STATS("onAudioStats"), ON_TRANSMISSION_STATISTICS_CHANGED("onTransmissionStatisticsChanged"), @Deprecated ON_QUALITY_CHANGED("onQualityChanged"), @Deprecated ON_NETWORK_HEALTH_CHANGED("onNetworkHealthChanged");
    ...
    public void init() {
        if (isInitialized()) {
            ...
            broadcastSession = new BroadcastSession(mReactContext, broadcastSessionListener, config, getInitialDeviceDescriptorList());
