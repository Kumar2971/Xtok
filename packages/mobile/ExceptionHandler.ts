...
const {ReactNativeExceptionHandler, RnTestExceptionHandler} = NativeModules;
...
export const setJSExceptionHandler = (
  ...
  if (allowed) {
    (global as any).ErrorUtils.setGlobalHandler(customHandler);
    const consoleError = console.error;
    console.error = (...args: any[]): void => {
      (global as any).ErrorUtils.reportError(...args);
      consoleError(...args);
    };
  } else {
    console.log(
...
export const getJSExceptionHandler = (): any =>
  (global as any).ErrorUtils.getGlobalHandler();

export const setNativeExceptionHandler = (
  customErrorHandler?: (errorString: string) => void,
